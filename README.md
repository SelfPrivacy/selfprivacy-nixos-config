...
nixosConfigurations-fun =
  ...
  , sp-modules             # flake references with sp-module- prefix from template
  }:
```

which returns one or more attributes, containing NixOS configurations (created with `nixpkgs.lib.nixosSystem`). (As of 2026-07-20 there is only a single configuration named `default`.)

## SelfPrivacy module schema

*How to add custom services to SelfPrivacy*

**Moved to [documentation on selfprivacy.org](https://selfprivacy.org/docs/theory/selfprivacy_modules/)**
...
- nixpkgs (following current stable NixOS release)
- selfprivacy-api (following latest SelfPrivacy API release)
...
In any case a Nix flake input is specified using some special _references_ syntax, including URLs, revisions, etc, described in manual: https://nixos.org/manual/nix/stable/command-ref/new-cli/nix3-flake.html#examples.
Such reference can be used inside `flake.nix` or as an argument to `nix flake` commands. When a new reference is encountered Nix downloads and extracts it to /nix/store.

Before and after running `nix flake lock` (or `nix flake update`) commands you would most likely want to list current inputs using `nix flake metadata`, which are read from `flake.lock` file.
Although, Nix should also print a diff between changed references once changed.
...
$ nix flake update selfprivacy-api
```
...
Example:
```console
$ nix flake lock --override-input nixpkgs 'github:nixos/nixpkgs?ref=nixos-26.05'
$ nix flake lock --override-input selfprivacy-api 'git+https://git.selfprivacy.org/SelfPrivacy/selfprivacy-rest-api.git?ref=master'
```

Similarly to update mechanism (described above), depending on the "precision" of an URL, its update scope varies accordingly.

Note, that subsequent calls of `nix flake lock --update-input <INPUT>` or `nix flake update` (or `nix flake update INPUT [INPUT2]`) will update the input regardless of the prior override.
The information about override is stored only in `flake.lock` (`flake.nix` is not altered by Nix).

Note, that override does not update flake inputs recursively (say, you have a flake nested inside your flake input). For recursive updates only `nix flake lock --update-input` and `nix flake update` mechanisms are suitable.
However, the SelfPrivacy NixOS configuration inputs don't contain references to other flakes, hence override mechanism is fine (don't confuse with top-level flake which has nested inputs of nixos-config and various sp modules).
`sp-modules/simple-nixos-mailserver` is an exception for this, as it depends on `simple-nixos-mailserver` flake.
